Java.perform(function() {
  ...
  const keyBytes = String.$new("your_secret_key_1234567890123456").getBytes();
  const ivBytes = String.$new("1234567890123456").getBytes();
  let cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
  cipher.init(0b10, SecretKeySpec.$new(keyBytes, "AES"), IvParameterSpec.$new(ivBytes));
  const path = Base64.encodeToString(cipher.doFinal(Base64.decode("bqGrDKdQ8zo26HflRsGvVA==", 0)), 0);
  ...
  setTimeout(function() {
    const pattern = "MHL{", module = Process.getModuleByName("libflag.so");
    let patternHex = [];
